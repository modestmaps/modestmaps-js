...
(function(MM) {
    ...
    MM.VectorMarkerLayer.prototype = {
        ...
        addMarker: function(marker, feature) {
            ...
            var loc = this.coerceLocation(feature),
                coord = this.map.locationCoordinate(loc);
            // convert the feature to a Location instance
            marker.data("location", loc);
            // remember the tile coordinate so we don't have to reproject every time
            marker.data("coord", coord);
            // update the marker's position
            this.repositionMarker(marker);
            // assume that the element has been added already
            // this.parent.appendChild(marker);
            // add it to the list
            this.markers.push(marker);
            return marker;
        ...
        removeMarker: function(marker) {
            ...
            // assume Raphael interface: Element.remove()
            marker.remove();
            return marker;
        },
    ...
    MM.extend(MM.VectorMarkerLayer, MM.MarkerLayer);
